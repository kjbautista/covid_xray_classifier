...
train_files, validation_files, train_labels, validation_labels=train_test_split(training_files,
    training_labels, test_size=0.20, random_state=24, 
...
for epoch in range(n_epochs):  # loop over the dataset multiple times
    ...
    for idx,data in enumerate(trainloader):
        ...
        running_accuracy.append(((sum(torch.argmax(out,1) == lab)/batch_size)).item())
        running_accuracy_epoch.append(((sum(torch.argmax(out,1) == lab)/batch_size)).item())
        if (idx+1) % 20 == 0: 
            print(f'[{epoch + 1}, {idx + 1:5d}] loss: {running_loss / 20:.3f}, accuracy: {np.mean(running_accuracy):.3f}')
...
#     scheduler.step()
    ...
    # iterate over validation data
        for data in validationloader:
            ...
            # running accuracy calculation
            running_accuracy_epoch.append(((sum(torch.argmax(out,1) == lab)/batch_size)).item())
    val_losses.append(running_loss_epoch / len(validationloader))
